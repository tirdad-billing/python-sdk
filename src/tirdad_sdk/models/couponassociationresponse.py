"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .couponresponse import CouponResponse, CouponResponseTypedDict
from .status import Status
from .subscriptionlineitemresponse import (
    SubscriptionLineItemResponse,
    SubscriptionLineItemResponseTypedDict,
)
from datetime import datetime
from pydantic import model_serializer
from tirdad_sdk.types import BaseModel, UNSET_SENTINEL
from typing import Dict, Optional
from typing_extensions import NotRequired, TypedDict


class CouponAssociationResponseTypedDict(TypedDict):
    coupon: NotRequired[CouponResponseTypedDict]
    coupon_id: NotRequired[str]
    created_at: NotRequired[datetime]
    created_by: NotRequired[str]
    end_date: NotRequired[datetime]
    r"""Optional"""
    environment_id: NotRequired[str]
    id: NotRequired[str]
    metadata: NotRequired[Dict[str, str]]
    start_date: NotRequired[datetime]
    status: NotRequired[Status]
    subscription_id: NotRequired[str]
    r"""Mandatory"""
    subscription_line_item: NotRequired[SubscriptionLineItemResponseTypedDict]
    subscription_line_item_id: NotRequired[str]
    r"""Optional"""
    subscription_phase_id: NotRequired[str]
    r"""Optional"""
    tenant_id: NotRequired[str]
    updated_at: NotRequired[datetime]
    updated_by: NotRequired[str]


class CouponAssociationResponse(BaseModel):
    coupon: Optional[CouponResponse] = None

    coupon_id: Optional[str] = None

    created_at: Optional[datetime] = None

    created_by: Optional[str] = None

    end_date: Optional[datetime] = None
    r"""Optional"""

    environment_id: Optional[str] = None

    id: Optional[str] = None

    metadata: Optional[Dict[str, str]] = None

    start_date: Optional[datetime] = None

    status: Optional[Status] = None

    subscription_id: Optional[str] = None
    r"""Mandatory"""

    subscription_line_item: Optional[SubscriptionLineItemResponse] = None

    subscription_line_item_id: Optional[str] = None
    r"""Optional"""

    subscription_phase_id: Optional[str] = None
    r"""Optional"""

    tenant_id: Optional[str] = None

    updated_at: Optional[datetime] = None

    updated_by: Optional[str] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(
            [
                "coupon",
                "coupon_id",
                "created_at",
                "created_by",
                "end_date",
                "environment_id",
                "id",
                "metadata",
                "start_date",
                "status",
                "subscription_id",
                "subscription_line_item",
                "subscription_line_item_id",
                "subscription_phase_id",
                "tenant_id",
                "updated_at",
                "updated_by",
            ]
        )
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m
